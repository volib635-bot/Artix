...
## 🛑 Challenge 2: Plaintext API Key Leakage (BYOK Security)

### Problem
Artix operates on a Bring-Your-Own-Key (BYOK) architecture. Storing raw API keys (OpenAI, Anthropic, Gemini) in plaintext inside browser `localStorage` created a security risk if an attacker extracted local storage or inspected unencrypted browser state.

### Technical Solution
Implemented automated **Client-Side Encryption & Key Obfuscation (`storage.ts` & `crypto.ts`)**:
...
2. **AES-256-GCM Encryption**: Users can enable Passphrase Encryption. Keys are encrypted using `AES-256-GCM` with a 100,000-iteration `PBKDF2` derived key.
3. **RAM Isolation**: In encryption mode, decrypted keys reside strictly in volatile JS memory (`memoryCache`) while the session is active and are wiped on page refresh.
